"""
Module for sending audio messages via WhatsApp Cloud API.

This module defines the AudioSender class, which provides methods to build payloads and send audio messages using the WhatsApp Cloud API.
"""
...
class AudioSender(LegacyMixinBase):
    """
    Mixin class for sending audio messages via WhatsApp Cloud API.

    Supports sending audio using either a previously uploaded media ID or a direct media link.
    """
    ...
    def _build_audio_payload(
        ...
        """
        Build the payload for sending an audio message.

        Args:
            media_id (str, optional): The media ID of the uploaded audio file.
            media_link (str, optional): The direct link to the audio file (not recommended).
    ...
    def send_audio_message(
        ...
        """
        Send an audio message to the recipient via WhatsApp Cloud API (synchronous).
        ...
        """
        payload = self._build_audio_payload(media_id=media_id, media_link=media_link)
        try:
            response = self._send_request(payload)
            response.raise_for_status()
            self.logger.info(f"Audio message sent to: {self.recipient_to_send}")
    ...
    async def async_send_audio_message(
        self,
        media_id: Optional[str] = None,
        media_link: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
        Send an audio message to the recipient via WhatsApp Cloud API (asynchronous).
